///
/// Auto-generated TCP server for optimizer: nmpc_uav_obstacle
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != NMPC_UAV_OBSTACLE_NUM_DECISION_VARIABLES {
    ...
    if let Some(y0) = &execution_parameter.initial_lagrange_multipliers {
        if y0.len() != NMPC_UAV_OBSTACLE_N1 {
    ...
    if parameter.len() != NMPC_UAV_OBSTACLE_NUM_PARAMETERS {
...
fn main() {
    let matches = App::new("OpEn TCP Server [nmpc_uav_obstacle]")
        .version("0.0.0")
        .author("John Smith")
