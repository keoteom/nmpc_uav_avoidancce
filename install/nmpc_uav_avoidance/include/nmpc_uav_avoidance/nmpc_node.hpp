...
constexpr int    NX       = 8;
constexpr int    NU       = 3;
constexpr int    N_PARAMS = 138;   // +3 for v_obs
...
private:
    ...
    // ---- Dynamic obstacle ----
    std::array<double, 3> obs_pos_{};     // position at current time
    std::array<double, 3> obs_vel_{};     // constant velocity  (NEW)
    double r_obs_          = 1.0;
    bool   obs_received_   = false;       // odometry received at least once
    ...
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr      sub_obs_odom_; // (NEW)
    ...
    void obs_odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg); // (NEW)
